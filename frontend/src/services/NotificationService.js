// Notification service for handling phone/SMS notifications
class NotificationService {
  ...
  static sendCampTodayNotifications(camp, registrations) {
    ...
    campRegistrations.forEach(registration => {
      ...
      const notification = {
        ...
        phoneNumber: registration.patientPhone || registration.phone,
        type: 'camp-today',
        title: 'Camp Today!',
        message: `Your registered camp "${camp.campName}" is happening today at ${camp.location.village}. Don't forget to attend!`,
        campId: camp.id,
  ...
  static sendCampTomorrowNotifications(camp, registrations) {
    ...
    campRegistrations.forEach(registration => {
      ...
      const notification = {
        ...
        type: 'camp-tomorrow',
        title: 'Camp Tomorrow!',
        message: `Reminder: Your camp "${camp.campName}" is tomorrow at ${camp.location.village}. Please be on time.`,
        campId: camp.id,
        campName: camp.campName,
        campDate: camp.date,
        campLocation: camp.location.village,
  ...
  static sendCampUpcomingNotifications(camp, registrations, days) {
    ...
    campRegistrations.forEach(registration => {
      ...
      const notification = {
        ...
        type: 'camp-upcoming',
        title: 'Upcoming Camp',
        message: `Your camp "${camp.campName}" is in ${days} days at ${camp.location.village}. Get ready for your health checkup.`,
        campId: camp.id,
        campName: camp.campName,
        campDate: camp.date,
        campLocation: camp.location.village,
  ...
  static sendSMSNotification(notification) {
    ...
    // Store SMS in phone notifications
    const smsNotification = {
      ...
      content: {
        ...
        patientName: 'Patient',
        campName: notification.campName,
        campDate: notification.campDate,
        campLocation: notification.campLocation,
    ...
    this.storeSMSNotification(smsNotification);
  }

  // Store SMS notification
  static storeSMSNotification(sms) {
    try {
      const smsNotifications = JSON.parse(localStorage.getItem('villageHealthSMS') || '[]');
      smsNotifications.push(sms);
      localStorage.setItem('villageHealthSMS', JSON.stringify(smsNotifications));
      console.log("📱 SMS notification stored:", sms);
    } catch (error) {
  ...
  static getMessagesForPhone(phoneNumber) {
    try {
      const smsNotifications = JSON.parse(localStorage.getItem('villageHealthSMS') || '[]');
      return smsNotifications.filter(sms => sms.phoneNumber === phoneNumber);
    } catch (error) {
  ...
  static markMessageAsRead(messageId) {
    try {
      const smsNotifications = JSON.parse(localStorage.getItem('villageHealthSMS') || '[]');
      const messageIndex = smsNotifications.findIndex(m => m.id === messageId);
      
      if (messageIndex !== -1) {
        smsNotifications[messageIndex].read = true;
        localStorage.setItem('villageHealthSMS', JSON.stringify(smsNotifications));
        return true;
      }
      return false;
  ...
  static deleteMessage(messageId) {
    try {
      const smsNotifications = JSON.parse(localStorage.getItem('villageHealthSMS') || '[]');
      const filteredMessages = smsNotifications.filter(m => m.id !== messageId);
      localStorage.setItem('villageHealthSMS', JSON.stringify(filteredMessages));
      return true;
    } catch (error) {
  ...
  // Send custom SMS message (for organizers)
  static sendCustomSMS(phoneNumber, message, subject = 'Health Camp Update') {
    const smsNotification = {
      id: this.generateSMSId(),
    ...
    this.storeSMSNotification(smsNotification);
    console.log(`📱 Custom SMS sent to ${phoneNumber}: ${message}`);
    return true;
  }
}
